...
export function TodaySignalStage() {
  ...
  function handleBandClick(event: ReactMouseEvent<HTMLButtonElement>, signal: Signal) {
    const target = event.target instanceof Element ? event.target : null;
    const gateClicked = Boolean(target?.closest(".fr-band-gate"));
    const active = opened && signal.rank === selectedRank;

    if (active && gateClicked) {
      event.preventDefault();
      window.location.assign(`/project/${encodeURIComponent(signal.itemId)}`);
      return;
    }

    if (!opened) {
      setSelectedRank(signal.rank);
      openToday();
      return;
    }
  ...
  return (
    <section className="fr-stack" data-open={opened ? "true" : "false"} data-switching={switching ? "true" : "false"} data-transition={transitionState} data-selected-lane={selectedSignal.lane}>
      ...
      <div className="fr-stack-shared" aria-label="Today signal stack">
          ...
          {SIGNALS.map((signal) => {
            ...
            return (
              <section className="fr-band" data-active={active ? "true" : "false"} data-lane={signal.lane} key={signal.rank}>
                <button className="fr-band-head" type="button" onClick={(event) => handleBandClick(event, signal)} aria-expanded={opened && active}>
