...
export function ExploreFieldV4({
  ...
  useEffect(() => {
    if (!focus) return;
    const node = document.querySelector<HTMLElement>(".lf4-sheet");
    if (!node) return;
  ...
  return (
    <section className="lf4" data-lens={lens}>
      <header className="lf4-hero">
        <div className="lf4-kicker">
          <span>02 EXPLORE · FRONTIER FIELD</span>
          <span>L12 TYPE COLONNADE</span>
          <span>{dataLabel}</span>
        </div>
        <div className="lf4-hero-grid">
          <h1>READ THE FIELD<br />AS RECORDS.</h1>
          <div className="lf4-intro">
            <p>
              Every hairline is one current candidate. It connects to a <strong>shared tag family taken from record metadata</strong>;
              rare or absent tags stay explicit in OTHER / UNTAGGED buckets. The left edge is ranked by the active Lens; Global Score never moves with the Lens.
            </p>
            <div className="lf4-hero-meta">
              <div><strong>{plotted.length}</strong><span>VISIBLE RECORDS</span></div>
      ...
      <figure className="lf4-canvas">
        <div className="lf4-canvas-head">
          <div>
            ...
            <span>vertical order = Lens rank · G = Global Score · L = current Lens score</span>
          </div>
          <div>
          ...
          {rows.map((row) => {
            ...
            return (
              <g
                ...
                onClick={() => setFocusId(row.candidate.itemId)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    setFocusId(row.candidate.itemId);
                  }
                }}
              >
        ...
        <figcaption>
          <span>Every line = one real candidate → one metadata-derived tag family; rare / missing tags are labeled, never inferred.</span>
          <span>No semantic coordinates. No embedding cluster claim. Lens changes rank only.</span>
        </figcaption>
