...
export function ProjectReadingController() {
  useEffect(() => {
    ...
    const initializeReadingState = (root: HTMLElement) => {
    ...
    const tryInitialize = () => {
      if (cancelled || readingCleanup) return;
      ...
      domObserver = null;
      readingCleanup = initializeReadingState(root);
    };

    tryInitialize();

    if (!readingCleanup) {
      domObserver = new MutationObserver(tryInitialize);
