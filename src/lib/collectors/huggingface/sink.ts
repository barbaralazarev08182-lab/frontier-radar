  ...
  async persistItem(input: HFPersistRepoInput): Promise<HFPersistOutcome> {
    ...
    const { id: itemId, inserted } = await upsertItem(
      ...
      // 由于 upsertItem 期望 NormalizedRepo 类型，这里做适配
      {
        ...
        fullName: n.fullName,
        primaryLanguage: n.pipelineTag ?? n.sdk ?? null, // 用 pipeline_tag 或 sdk 作为"语言"
        license: null,
        defaultBranch: null,
        homepageUrl: null,
        createdAt: n.createdAt ?? new Date().toISOString(),
        pushedAt: n.updatedAt ?? new Date().toISOString(),
        stars: n.likes ?? 0, // HF 用 likes 类比 stars
        forks: 0,
        watchers: 0,
        openIssues: 0,
        subscribers: null,
    ...
    await insertSnapshot(this.supabase, {
      ...
      stars: n.likes, // HF likes → stars 列（复用现有列）
      forks: null,
      watchers: null,
      open_issues: null,
      subscribers: null,
